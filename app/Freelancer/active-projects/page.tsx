...
export default function ActiveProjects() {
    ...
    // Filter for current user's SELECTED projects
    const activeProjects = applications.filter(
        app => app.applicantName === "Sarah Johnson" && app.status === "Selected"
    );
        ...
        <div className="p-8 bg-slate-50 min-h-screen">
            ...
            <div className="mb-10 flex justify-between items-end">
                <div>
                    ...
                    <p className="text-gray-500 font-medium">Projects you are currently working on</p>
                </div>
                <div className="bg-emerald-50 px-4 py-2 rounded-xl border border-emerald-100 flex items-center gap-2">
                    <Activity className="w-4 h-4 text-emerald-600" />
                    <span className="text-emerald-600 font-black text-xs uppercase tracking-widest">{activeProjects.length} Ongoing</span>
                </div>
            </div>
            ...
            {activeProjects.length > 0 ? (
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                    {activeProjects.map((app) => (
                        ...
                        >
                            {/* Accent Background */}
                            <div className="absolute top-0 right-0 w-32 h-32 bg-blue-50/50 rounded-bl-full -mr-10 -mt-10 transition-transform group-hover:scale-110" />

                            <div className="relative z-10">
                                <div className="flex justify-between items-start mb-6">
                                    <div className="w-16 h-16 bg-blue-50 rounded-[24px] flex items-center justify-center text-blue-600 font-black text-2xl border border-blue-100">
                                        {app.projectTitle[0]}
                                    </div>
                                ...
                                <p className="text-gray-400 font-bold text-sm mb-8 uppercase tracking-widest">Selected on: {app.appliedDate}</p>

                                <div className="grid grid-cols-2 gap-4 mb-8">
                                    <div className="p-4 bg-slate-50 rounded-2xl border border-gray-50">
                                        <div className="flex items-center gap-2 mb-1">
                                            <DollarSign className="w-4 h-4 text-gray-400" />
                                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Project Budget</span>
                                        </div>
